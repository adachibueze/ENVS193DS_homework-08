...
drought_exp <- read_xlsx(path = here("data", # folder data file is in
                                     "Valliere_etal_EcoApps_Data.xlsx"), #what file actual is
                         sheet = "First Harvest") #specific sheet it named

# quick look at data 
str(drought_exp) # tells you about the structure of the data
class(drought_exp) #tells you what kind of thing object is
```
...
#output
# density curves desribes distribution of each variable
# on the bottom left have 10 different geom point syle plot for reprsenting relationships between variable
# also see the correlation between variables as well
# quick way to show relationships between variables, that helps show  relationship between the data
```

Choose 3 pairs of variables to compare. Make notes of those comparisons below.    

**leaf dry weight is strongly postively correlated with total mass (Pearson r= 0.46)**  

**shoot mass and total mass are strongly positively correlated (Pearson r = 0.92)**  

**total mass and root mass are strongly positively correlated (Pearson r= 0.858)**
...
ggplot(data = drought_exp_clean, # data frame
       aes(x = reorder(species_name, # reordering x-axis
                       -total_g, # in reverse order of mean total mass, helps to see a pattern
...
**gumweed, california poppy, and purple needlegrass seem to have the highest biomass; deerweed, scarlet bugler, and purple needle grass seem to have the lowest biomass**
...
**The relationship appears to be rather unclear on how SLA influences total biomas, however do see a slight positive correlation. Modertate or strong correlation relationhip depending on how you round**
...
create this model on own own for hw
...
model_dt <- read_csv("/Users/adachibueze/github/ENVS193DS_workshop-08/model_dt.csv") #reading in                                                                                        #data frame to                                                                                      #use it to                                                                                         #make a table 
model_table<-flextable(model_dt) #using flextable package to make table
model_table <- add_header_row( #adding header row called
  model_table, 
  values = c("", "Predictor Variables"), 
  colwidths = c(1, 3))
print(model_table)
...
```{r}
model.sel(model0, #takes all model objects and models them in a table
          model1, 
          model2, 
          model3)
# delta is difference between best model and all other model, the delta AIC for best model is always going to be zero
#need to choose best model based on AIC model


```
**Spefic leaf area, water treatment and species** are the variables of the best moodel
...
- reference for `water_treatment` --> reference level is whatever does not appear in table for categorical table
- reference for `species_name` -->  reference level is whatever does not appear in table 

**reference for water_treatment: is drought stressed**  
**reference for species_name:Acmipson glaber does not appear**  

Interpret the model estimates for the:  
**mass of the reference (Acmispon glaber) in drought stressed water treament when SLA = 0**  
- SLA slope: **FOR EACH unit increase in sla (mm^2/g) you expect a decrease in plant mass of 0.002, NO EFFECT OF SLA ON MASS (high p-value)**  
- well watered estimate: **on average across species and SLA, well watered plants tend to be 0.12 plus minus 0.02g heavier then drought treatment plants**    
