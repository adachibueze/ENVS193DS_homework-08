...
```{r}
model_preds_for_plot_4 <- model_preds_4 %>%
...
ggplot() + 
  ...
  geom_ribbon(data = model_preds_for_plot_4, 
              aes(x = species_name, 
                  ymin = conf.low, 
                  ymax = conf.high, 
              ...
              alpha = 0.2) + 
  # Model predictions
  geom_line(data = model_preds_for_plot_4, 
            aes(x = species_name, 
                y = predicted, 
                color = water_treatment)) +
  theme_minimal() +
  ...
  labs(title = "Model Predictions with 95% CI",
       x = "Species Name",
       y = "Predicted Total G",
